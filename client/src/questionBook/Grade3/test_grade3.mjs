import Grade3Questions from './GetGrade3Question.mjs';

console.log("Testing Grade 3 Questions...");
...
const checkQuestion = (key, question, index) => {
    ...
    if (question.type === 'mcq') {
        ...
        // Exceptions: Symmetry (Yes/No), Compare Fractions (>, <, =)
        let expectedOptions = 4;
        if (question.topic.includes("Symmetry")) expectedOptions = 2;
        if (question.topic.includes("Compare")) expectedOptions = 3;
...
for (const [key, questions] of Object.entries(Grade3Questions)) {
    process.stdout.write(`Checking ${key} (${questions[0]?.topic || 'Unknown'})... \r`);
